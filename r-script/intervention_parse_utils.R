...
# New JSONL rows include logp_delta_convention = "intervened_minus_baseline".
# Older files stored baseline - intervened; flip so plots match the same semantics.
normalize_logp_delta_columns <- function(df) {
  legacy <- !("logp_delta_convention" %in% names(df)) ||
    !any(df$logp_delta_convention == "intervened_minus_baseline", na.rm = TRUE)
  if (legacy) {
    if ("teacher_forced_delta_sum" %in% names(df)) {
      df$teacher_forced_delta_sum <- -as.numeric(df$teacher_forced_delta_sum)
    }
    if ("delta_logp_final_token" %in% names(df)) {
      df$delta_logp_final_token <- -as.numeric(df$delta_logp_final_token)
    }
    message("Note: legacy JSONL (no logp_delta_convention=intervened_minus_baseline); ",
            "flipped logp columns to intervened - baseline.")
  }
  df
}
