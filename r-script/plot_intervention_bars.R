...
# Plot per-step intervention bars (delta logp, delta acc, flip rate) across models.
...
read_jsonl_df <- function(path) {
  ...
  normalize_logp_delta_columns(as_tibble(df))
}
...
for (model in names(opts$inputs)) {
  ...
  df <- df %>%
    mutate(
      ...
      intervened_ans = extract_answer(intervened_text),
      gold_num = extract_number(gold_raw),
      baseline_num = extract_number(baseline_text),
      intervened_num = extract_number(intervened_text),
      ...
      intervened_correct = ifelse(!is.na(gold_num) & !is.na(intervened_num),
                               abs(intervened_num - gold_num) < 1e-6,
                               !is.na(gold_ans) & !is.na(intervened_ans) & gold_ans == intervened_ans)
    )
  ...
  acc_tbl <- df_mode %>%
    ...
    summarise(
      ...
      intervened_acc = ifelse(total > 0, correct / total, NA_real_),
      .groups = "drop"
    ) %>%
    left_join(baseline_tbl, by = "step_i") %>%
    mutate(delta_acc = intervened_acc - baseline_acc)
...
out_dir <- if (is.null(opts$out_dir)) "outputs/plots/intervention_bars" else opts$out_dir
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

p_logp <- plot_bars(
  logp_plot,
  "Delta log(p) of the answer by steps",
  "Δ logp (intervened - baseline)"
)
p_acc <- plot_bars(
  acc_plot,
...
ggsave(
  file.path(out_dir, "intervention_bars_triptych.png"),
  combined,
  width = 19,
  height = 6.2,
  dpi = 300,
