...
# GSM8K-only flip-rate bars for four Coconut models (same styling as plot_fliprate_steps_bars.R).
...
parse_args <- function(args) {
  opts <- list(gsm8k = list(), out_dir = NULL, file_stub = "plot_fliprate_steps_bars_for_rebuttal", mode = "zero")
  i <- 1
  while (i <= length(args)) {
    key <- args[[i]]
    if (key == "--gsm8k") {
  ...
  if (length(opts$gsm8k) == 0) {
    stop("Provide at least one --gsm8k label=path input.")
  }
  opts
}
...
# Must match --gsm8k label=... keys in scripts/plot/r/rq1/plot_fliprate_steps_bars_for_rebuttal.sh exactly,
# or factor() drops rows and bars disappear from the plot.
model_levels <- c(
  "Coconut-Qwen3-1.7B",
  "Coconut-Mistral-7B",
  "Coconut-DeepSeek-R1-Qwen2.5-1.5B",
  "Coconut-Llama3-8B"
)

color_map <- c(
  "Coconut-Qwen3-1.7B" = "#c6dbef",
  "Coconut-Mistral-7B" = "#9ecae1",
  "Coconut-DeepSeek-R1-Qwen2.5-1.5B" = "#4292c6",
  "Coconut-Llama3-8B" = "#084594"
)
...
ggsave(pdf_path, gsm8k_plot, width = 7.2, height = 3.6, dpi = 300, limitsize = FALSE)
message("Saved: ", pdf_path)

if (requireNamespace("svglite", quietly = TRUE)) {
  ggsave(svg_path, gsm8k_plot, width = 7.2, height = 3.6, dpi = 300, limitsize = FALSE)
