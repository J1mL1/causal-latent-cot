...
# R port of scripts/plot/python/rq1/analyze_intervention_jsonl.py
...
df <- normalize_logp_delta_columns(df)
...
df <- df %>%
  mutate(
    ...
    intervened_pred = sapply(intervened_text, extract_number),
    ...
    intervened_correct = ifelse(
      ...
      !is.na(intervened_pred) & !is.na(gold_num) & abs(intervened_pred - gold_num) < 1e-6
    )
  )
...
intervened_stats <- df %>%
  ...
  summarise(
    total = sum(!is.na(intervened_correct)),
    correct = sum(intervened_correct, na.rm = TRUE),
    acc = ifelse(total > 0, correct / total, NA_real_),
    .groups = "drop"
  ) %>%
...
flip_stats <- df %>%
  filter(!is.na(step_i), !is.na(baseline_correct), !is.na(intervened_correct)) %>%
  group_by(mode, step_i) %>%
  summarise(
    total = n(),
    correct_to_wrong = sum(baseline_correct & !intervened_correct, na.rm = TRUE),
    wrong_to_correct = sum(!baseline_correct & intervened_correct, na.rm = TRUE),
...
if (nrow(baseline_tbl) == 0) {
  ...
  row <- baseline_tbl[1, ]
  cat(sprintf("%s\n", format_bucket(row$total, row$correct)))
}

cat("\n=== Intervened (greedy accuracy & flips) ===\n")
if (nrow(intervened_stats) > 0) {
  modes_present <- sort(unique(intervened_stats$mode))
...
if (nrow(logp_stats) > 0) {
  ...
  cat("Positive: higher log p on gold after intervention. Negative: lower.\n")
...
if (nrow(logp_stats) > 0) {
  ...
  plot_heatmap(
    mat_seq,
    "Mean Δ log p(gold sequence) (intervened - baseline)",
    "Δlogp_seq",
  ...
  plot_heatmap(
    mat_final,
    "Mean Δ log p(final gold token) (intervened - baseline)",
    "Δlogp_final",
