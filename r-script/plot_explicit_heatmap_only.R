...
build_explicit_aggregates <- function(df, metric) {
  ...
  list(steps = steps, step_tbl = step_tbl)
}

plot_heatmap <- function(steps, step_tbl, out_path, metric_label = "kl_mean") {
  mat <- step_tbl %>%
    ...
    mutate(weight = ifelse(step_i == step_j, NA_real_, weight))
...
main <- function() {
  ...
  agg <- build_explicit_aggregates(df, opts$metric)
  steps <- agg$steps
  ...
  plot_heatmap(steps, agg$step_tbl, opts$out_path, metric_label = opts$metric)
}
