...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run latent step interventions.")
    ...
    parser.add_argument(
        "--only_estimate_mean",
        action="store_true",
        help="Only compute and cache mean latents, then exit (no intervention).",
    )
            ...
            for target_len, items in batched_by_len.items():
                ...
                for step in tqdm(steps, desc="steps", leave=False, disable=rank != 0):
                    ...
                    for mode in tqdm(modes, desc="modes", leave=False, disable=rank != 0):
                        # Skip per-step mean interventions when we do not have an estimated mean for that step.
                        if mode in {"mean_step", "gaussian_mu_step"} and (
                            mu_by_step is None or step not in mu_by_step
                        ):
                            continue
                        result = run_step_intervention(
                            model,
                            prompts,
                            step_t=step,
                            mode=mode,
                        ...
                        intervened_state = build_teacher_state(result["intervened"], result.get("state"))
                        if rank == 0 and sample_counter <= 1 and step == steps[0] and mode == modes[0]:
                            print(f"DEBUG: using cache source baseline={baseline_state.get('_cache_source') if baseline_state else None} intervened={intervened_state.get('_cache_source') if intervened_state else None}")
                            ...
                            if intervened_state and "past_key_values_latents" in result.get("intervened", {}):
                                print("DEBUG: intervened includes past_key_values_latents")
                        tf_metrics = {}
                        if baseline_logits is not None and intervened_state is not None:
                            try:
                                ablt_logits = model.compute_logits(result["h_t_modified"], intervened_state, target_ids)
                        ...
                        for idx, item in enumerate(items):
                            ...
                            intervened_clean = strip_caches(_select_sample_output(result["intervened"], idx))
                            record = {
                                ...
                                "intervened": summarize_output(intervened_clean),
                                "h_t_shape": list(result["h_t"].shape)
                                if isinstance(result.get("h_t"), torch.Tensor)
                                else None,
                                "logp_delta_convention": LOGP_DELTA_CONVENTION,
                            }
            ...
            for item in per_sample:
                ...
                for step in tqdm(steps, desc="steps", leave=False, disable=rank != 0):
                    ...
                    for mode in tqdm(modes, desc="modes", leave=False, disable=rank != 0):
                        ...
                        intervened_clean = strip_caches(result["intervened"])
                        record = {
                            "sample_id": item["sample_id"],
                            "sample_uid": item["sample_id_field"],
                            "batch_idx": 0,
