#!/usr/bin/env python
"""
Quick analyzer for intervention JSONL outputs.

It streams the JSONL file and reports:
  - numeric accuracy for baseline/intervened texts per (mode, step)
  - flip rates: baseline correct -> intervened wrong and baseline wrong -> intervened correct
  - teacher-forced log-prob deltas per (mode, step), same sign as delta_acc:
        * teacher_forced_delta_sum       (Σ_t log p_int(y_t) - log p_base(y_t) over gold answer tokens)
        * delta_logp_final_token         (log p_int(y*) - log p_base(y*) on last gold token)
...
Usage:
  python scripts/plot/python/rq1/analyze_intervention_jsonl.py --path outputs/rq1/intervention/gsm8k_coconut_gpt2.jsonl
"""
...
def summarize_delta_heatmap(
    intervened_stats: Dict[Tuple[str, int], Dict[str, float]],
    baseline_acc: float,
) -> Tuple[List[str], List[int], List[List[float]], List[List[str]]]:
    modes = sorted({m for (m, _) in intervened_stats.keys()})
    steps = sorted({s for (_, s) in intervened_stats.keys()}, key=step_order_key)
...
def main() -> None:
    ...
    parser.add_argument("--path", required=True, help="Path to intervention JSONL.")
    ...
    with open(args.path, "r", encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            if rec0.get("logp_delta_convention") == "intervened_minus_baseline":
                logp_sign = 1.0
            break
    if logp_sign < 0:
        print(
            "Note: legacy JSONL (no logp_delta_convention=intervened_minus_baseline); "
            "flipping logp columns to intervened - baseline for plots."
        )
    ...
    with open(args.path, "r", encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            # ---- intervened stats per (mode, step) ----
            ...
            if intervened_texts:
                ...
                if text_idx < 0 or text_idx >= len(intervened_texts):
                    text_idx = 0
                pred_val = extract_number(intervened_texts[text_idx])
                update_acc_stats(intervened_stats, key, gold_val, pred_val)
            ...
            if delta_seq is not None:
                ...
                lb["count_seq"] += 1.0
                lb["sum_delta_seq"] += logp_sign * float(delta_seq)
            ...
            if delta_final is not None:
                ...
                lb["count_final"] += 1.0
                lb["sum_delta_final"] += logp_sign * float(delta_final)
    ...
    print("\n=== Intervened (greedy accuracy & flips) ===")
    modes_present = sorted({m for (m, _) in intervened_stats.keys()})
    if modes_present:
        print("Mode descriptions:")
        for m in modes_present:
            print(f"  {m}: {MODE_DESCRIPTIONS.get(m, 'N/A')}")

    for (mode, step) in sorted(intervened_stats.keys(), key=lambda t: (t[0], step_order_key(t[1]))):
        print(
            f"{mode:15s} step={step}: "
            f"{format_bucket(intervened_stats[(mode, step)])} | "
            f"{format_flip(flip_stats.get((mode, step), flip_bucket_default()))}"
        )

    if logp_stats:
        print("\n=== Teacher-forced logp deltas (intervened - baseline) ===")
        print("Positive: higher log p on gold after intervention. Negative: lower.")
    ...
    if intervened_stats:
        rows, steps, grid, ann = summarize_delta_heatmap(intervened_stats, baseline_acc)
        plot_heatmap(
            grid, rows, steps,
    ...
    if logp_stats:
        ...
        plot_heatmap(
            grid, rows, steps,
            title="Mean Δ log p(gold sequence) (intervened - baseline)",
            cmap="RdBu_r",
            annotations=ann,
        ...
        plot_heatmap(
            grid, rows, steps,
            title="Mean Δ log p(final gold token) (intervened - baseline)",
            cmap="RdBu_r",
            annotations=ann,
